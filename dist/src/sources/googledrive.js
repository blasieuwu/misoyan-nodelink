 ...
 * Follows HttpSource pattern for robust format resolution and streaming.
 * @public
 */
    ...
    async loadStream(_track, url) {
        ...
        const tryReconnect = async (reason) => {
            ...
            if (reconnectAttempts >= maxReconnectAttempts) {
                logger('debug', 'GoogleDrive', `Stream disconnected (${reason}) after ${reconnectAttempts} retries, ending stream.`);
                finalStream.emit('finishBuffering');
                finalStream.end();
                return;
            }
            reconnecting = true;
            reconnectAttempts++;
            logger('debug', 'GoogleDrive', `Stream disconnected (${reason}), retrying ${reconnectAttempts}/${maxReconnectAttempts} from byte ${totalSourceBytesRead}.`);
